...
# Load data
data = load("data/poisson_data.jld")
X_data = data["X"]
Y_data = data["S"]
w_true = data["w"]
(m,d) = size(X_data)
...
function likelihood_per_point(w)
    return (exp.(Y_data.*(X_data*w)).*(exp.(-exp.(X_data*w))))./(factorial.(Y_data))
end
...
for run in 1:total_runs
    ...
    G_x_bar = likelihood_per_point(x_bar)
    inner_func_eval_count += 1
    
    # Evaluate loss function
    D_x_bar = -sum(log.(G_x_bar)) 
    outer_func_eval_count += 1
    
    # Evaluate objective function
    phi_x_bar = D_x_bar + (lambda_x/2)*(x_bar'*B*x_bar)
    ...
    for i in 1:maxIter
        ...
        while ls_flag == false
            ...
            if line_search_fail >= max_backtracks
                ...
                G_x_bar_new = likelihood_per_point(x_bar_new)
                inner_func_eval_count += 1
                
                D_x_bar_new = -sum(log.(G_x_bar_new))
                outer_func_eval_count += 1
                
                phi_x_bar_new = D_x_bar_new + (lambda_x/2)*(x_bar_new'*B*x_bar_new)
            ...
            T_inv = I + (scaled_dt_prop/K)*(G_devs'*H_loss_x_bar*G_devs + (lambda_x/2)*Y'*(B+B')*Y)
            if verbose
                @show T_inv
            ...
            G_x_bar_prop = likelihood_per_point(x_bar_prop)
            inner_func_eval_count += 1
            
            # Evaluate loss function
            D_x_bar_prop = -sum(log.(G_x_bar_prop))
            outer_func_eval_count += 1
            
            # Evaluate objective function
            phi_x_bar_prop = D_x_bar_prop + (lambda_x/2)*(x_bar_prop'*B*x_bar_prop)
...
figure()
axes_nll = plt.gca()
axes_nll.set_xlim([0,pltIter])
axes_nll.set_ylim([2,3])
axes_nll.grid(true)
axes_nll.tick_params(axis="x", labelsize=26)
axes_nll.tick_params(axis="y", labelsize=26)
xlabel(L"Iteration $n$", fontsize=26)
ylabel(L"$\log_{10} \bigg ( \Phi(\mathbf{\bar{x}}_{n}) \bigg )$", fontsize=26)
title("EnKSGD NLL", fontsize=26)
all_true_obj_fun_vals = zeros(maxIter+1,total_runs)
for run in 1:total_runs
    for j in 1:(maxIter+1)
        all_true_obj_fun_vals[j,run] = -sum(log.(likelihood_per_point(all_x_vals[j,:,run])))
    end
    axes_nll.plot(0:pltIter,log.(10,all_true_obj_fun_vals[:,run]), alpha=0.8, color=color_map(run/total_runs), label="EnKSGD")
end
axes_nll.text(-0.3, 1.1, "(A)", size=20, weight="bold", transform=axes_nll.transAxes)
axes_nll.yaxis.set_label_coords(-.2, 0.5)
