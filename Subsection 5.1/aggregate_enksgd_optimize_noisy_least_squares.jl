...
title("EnKSGD Gaussian Noise", fontsize=26)
...
for run in 1:total_runs
    ...
    axes_opt_gap.plot(0:pltIter,log.(10,all_true_obj_fun_vals[:,run]), alpha=0.8, color=color_map(run/total_runs), label="EnKSGD")
end
axes_opt_gap.text(-0.3, 1.1, "(E)", size=20, weight="bold", transform=axes_opt_gap.transAxes)
