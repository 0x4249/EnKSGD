...
title("EnKF No Noise", fontsize=26)
for run in 1:total_runs
    axes_opt_gap.plot(0:pltIter,log.(10,all_phi_vals[:,run]), alpha=0.8, color=color_map(run/total_runs), label="EnKF")
end
axes_opt_gap.text(-0.3, 1.1, "(C)", size=20, weight="bold", transform=axes_opt_gap.transAxes)
